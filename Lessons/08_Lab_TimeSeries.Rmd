...
Rainfall_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>%  # separate into 3 columns, 
  # the first "Null" is the space in front of month, "null2" is average, separate by space "" 
...
rainfall <- ggplot(BR_complete, aes(x = Date, y = Rainfall)) +
  geom_line() +
  geom_smooth(method = lm)
rainfall

temp <- ggplot(BR_complete, aes(x = Date, y = Temperature_C)) +
  geom_line() +
  geom_smooth(method = lm)
temp
...
BR.rain_ts <- ts(BR_complete$Rainfall, start = c(1901, 1), frequency = 12)
BR.temp_ts <- ts(BR_complete$Temperature_C, start = c(1901, 1), frequency = 12)
...
BR.rain_Decomposed <- stl(BR.rain_ts, s.window = "periodic")
BR.temp_Decomposed <- stl(BR.temp_ts, s.window = "periodic")

# Visualize the decomposed series. 
plot(BR.rain_Decomposed)
plot(BR.temp_Decomposed)
...
BR.rain_Components <- as.data.frame(BR.rain_Decomposed$time.series[,1:3])
BR.temp_Components <- as.data.frame(BR.temp_Decomposed$time.series[,1:3])

BR.rain_Components <- mutate(BR.rain_Components,
        Observed = BR_complete$Rainfall,     
        Date = BR_complete$Date)

BR.temp_Components <- mutate(BR.temp_Components,
        Observed = BR_complete$Temperature_C,     
        Date = BR_complete$Date)


# Exercise 10: Visualize how the trend maps onto the data for both series
ggplot(BR.rain_Components, aes(x = Date)) +
  geom_line(aes(y = Observed), col = "black") +
  geom_line(aes(y = trend), col = "red") +
  ylab("Rainfall")

ggplot(BR.temp_Components, aes(x = Date)) +
  geom_line(aes(y = Observed), col = "black") +
  geom_line(aes(y = trend), col = "red") +
  ylab("Temperature")
...
temp.trend <- trend::smk.test(BR.temp_ts)
summary(temp.trend)
